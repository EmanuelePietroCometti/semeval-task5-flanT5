...
Let **$p \in \mathbb{R}^5$** be the softmax probability distribution over the tokens **$\{1, 2, 3, 4, 5\}$** generated by the model.
The continuous predicted score **$\hat{y}$** is the expected value:
...
$$
\text{Accuracy} = \frac{1}{N} \sum_{i=1}^{N} \mathbb{1}\left( | \hat{y}_i - y_i | \le \max(1.0, \sigma_i) \right)
